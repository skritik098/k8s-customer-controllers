...
func main() {
	...
	if err != nil {
		...
		fmt.Printf("error %s building the config from kubeconfig\n", err.Error())

		// Here if the config read is failing then probably we assuming this app is running on cluster
		// As we know when a pod is running in k8s cluster, a default SA gets mounted inside that pod
		config, err := rest.InClusterConfig()
		if err != nil {
			// Handle error
			fmt.Printf("error %s while reading service account", err.Error())
		}
	}
	...
	if err != nil {
		...
		fmt.Printf("error %s building a clientset from config\n", err.Error())
	}
	...
	if err != nil {
		//Handle error
		fmt.Printf("error %s getting pod list from kube-system namespace\n", err.Error())
	}
